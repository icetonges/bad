...
export async function POST(req: NextRequest) {
  try {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          ...
          send({ type: 'error', message: err?.message || String(err) } as any)
        } finally {
          controller.close()
        }
      },
    ...
    if (/relation .* does not exist/i.test(msg)) {
      hint = ' (Database tables are missing — run `npm run db:migrate` against the production DATABASE_URL.)'
    } else if (/DATABASE_URL/i.test(msg)) {
      hint = ' (DATABASE_URL env var is missing or invalid in Vercel.)'
    } else if (/API_KEY/i.test(msg)) {
      hint = ' (AI provider API key is missing in Vercel env vars.)'
    }
